...
class WeekImagePage {
    ...
    setupEventListeners() {
        ...
        document.getElementById('unselect-all-tracks').addEventListener('click', () => {
        ...
        // Track meta toggle
        document.getElementById('show-track-meta').addEventListener('change', (e) => {
            this.toggleTrackMeta(e.target.checked);
        });
        ...
        document.querySelectorAll('.preset-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                // Check if this is a custom preset with additional parameters
                if (e.target.classList.contains('preset-custom') || e.target.dataset.hscroll !== undefined) {
                    ...
                    const hspacing = parseInt(e.target.dataset.hspacing);
                    const vspacing = parseInt(e.target.dataset.vspacing);
                    const scale = parseInt(e.target.dataset.scale);
                    const textsize = parseInt(e.target.dataset.textsize);
                    ...
                    if (!isNaN(hscroll)) this.setHorizontalScroll(hscroll);
                    if (!isNaN(vpos)) this.setVerticalPosition(vpos);
                    if (!isNaN(spacing)) this.setGridGap(spacing);
                    if (!isNaN(hspacing)) this.setGridGapHorizontal(hspacing);
                    if (!isNaN(vspacing)) this.setGridGapVertical(vspacing);
                    if (!isNaN(scale)) this.setContentScale(scale);
                    if (!isNaN(textsize)) this.setTextSize(textsize);
                    if (!isNaN(cardsize)) this.setCardSize(cardsize);
                    if (!isNaN(columns)) this.setGridColumns(columns);
                    
                    // Update UI sliders
                    if (!isNaN(hscroll)) document.getElementById('horizontal-scroll-slider').value = hscroll;
                    if (!isNaN(vpos)) document.getElementById('vertical-position-slider').value = vpos;
                    if (!isNaN(spacing)) {
                        const slider = document.getElementById('grid-gap-slider');
                        if (slider) slider.value = spacing;
                    }
                    if (!isNaN(hspacing)) {
                        const slider = document.getElementById('grid-gap-horizontal-slider');
                        if (slider) slider.value = hspacing;
                    }
                    if (!isNaN(vspacing)) {
                        const slider = document.getElementById('grid-gap-vertical-slider');
                        if (slider) slider.value = vspacing;
                    }
                    if (!isNaN(scale)) document.getElementById('content-scale-slider').value = scale;
                    if (!isNaN(textsize)) document.getElementById('text-size-slider').value = textsize;
                    if (!isNaN(cardsize)) document.getElementById('card-size-slider').value = cardsize;
                    if (!isNaN(columns)) document.getElementById('columns-slider').value = columns;
                }
            });
        });
    ...
    selectAllTracks() {
        // Add all tracks to selectedTracks
        Object.keys(this.tracks).forEach(trackId => {
            this.selectedTracks.add(trackId);
        });
        
        // Update all checkboxes
        document.querySelectorAll('#track-selector input[type="checkbox"]').forEach(checkbox => {
    ...
    renderTracks() {
        ...
        this.selectedTracks.forEach(trackId => {
            ...
            trackCard.innerHTML = `
                <img src="/api/assets/covers/${track.cover}" alt="${track.title}" class="track-cover" onerror="this.src='/api/assets/images/logo.png'">
                <div class="track-info">
                    <h3>${track.title}</h3>
                    <p>${track.artist}</p>
                    <div class="track-meta" style="display: ${this.showTrackMeta ? 'block' : 'none'}">
    ...
    showTrackModal(trackId) {
        ...
        modalBody.innerHTML = `
            <div class="modal-track-info">
                <img src="/api/assets/covers/${track.cover}" alt="${track.title}" class="modal-cover" onerror="this.src='/api/assets/covers/fallback.jpg'">
                <div class="modal-details">
                    ...
                    ${track.previewUrl ? `
                        <div class="audio-preview">
                            <audio controls>
                                <source src="/api/assets/audio/${track.previewUrl.split('/').pop()}" type="audio/mpeg">
    ...
    setBackground(bgType) {
        // Fallback for deprecated background styles
        if (bgType === 'weekimage' || bgType === 'festival' || !bgType) {
            bgType = 'gradient';
        }
        ...
        const btn = document.querySelector(`[data-bg="${bgType}"]`);
    ...
    toggleTrackMeta(show) {
        this.showTrackMeta = show;
        this.renderTracks();
        this.saveSettings();
    }
    ...
    saveSettings() {
        const settings = {
            ...
            showTrackMeta: this.showTrackMeta,
        ...
        localStorage.setItem('weekimageSettings', JSON.stringify(settings));
    }

    loadSettings() {
        const saved = localStorage.getItem('weekimageSettings');
        if (!saved) return;
        ...
        try {
            ...
            if (settings.showTrackMeta !== undefined) {
                this.showTrackMeta = settings.showTrackMeta;
                document.getElementById('show-track-meta').checked = settings.showTrackMeta;
            }
...
// Initialize the weekimage page when DOM is loaded
